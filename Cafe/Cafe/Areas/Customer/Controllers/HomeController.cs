...
namespace Cafe.Areas.Customer.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> Rezervasyon([Bind("Id,Name,Email,TelefonNo,Sayi,Saat,Tarih")] Rezervasyon rezervasyon)
        {
            if (ModelState.IsValid)
            {
                // Aynı tarih ve saat için kayıt var mı kontrolü
                var mevcutRezervasyon = _db.Rezervasyons
                    .FirstOrDefault(r => r.Tarih.Date == rezervasyon.Tarih.Date && r.Saat == rezervasyon.Saat);

                if (mevcutRezervasyon != null)
                {
                    _toast.AddErrorToastMessage("Üzgünüz, bu tarih ve saatte zaten bir rezervasyon var.");
